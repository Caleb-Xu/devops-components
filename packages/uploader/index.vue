        ...
        <div class="all-file" v-if="fileList.length">
            <div v-for="(file, index) in fileList" :key="index">
                <div :class="{ 'file-item-fail': file.errorMsg }" class="file-item">
                    ...
                    <i v-if="file.errorMsg" class="bk-icon icon-refresh close-upload" title="重试"
                        @click="retry(index, file)"></i>
                    <i v-if="file.done && quickRemove" class="bk-icon icon-delete close-upload" title="删除"
                        @click="removeFile(index, file)"></i>
                    <div class="file-info">
                        <div class="file-name"><span>{{ file.name }}</span></div>
                        <div class="file-message">
                            <span class="upload-speed" v-show="!file.done && file.status === 'running'">{{ speed }}{{ unit
                            }}</span>
                        ...
                        <div v-else class="progress-bar-wrapper">
                            ...
                            }" class="progress-bar" :style="{ width: file.progress }">
                            </div>
                        </div>
                    </div>
                </div>
...
import { defaultRequest, FileObj, FormDataAttributes, sliceRequest, UploaderOptions, XhrResponse } from './request'
...
function t(str: keyof typeof uploaderI18n) {
...
export default defineComponent({
    ...
    props: {
        ...
        handleResCode: {
            type: Function,
            default: () => {
                return function (res: XhrResponse) {
                    return res?.code === 0
                }
            }
        ...
        customRequest: {
            type: Function as PropType<(options: UploaderOptions) => unknown>
        },
        sliceUpload: {
            type: Boolean,
            default: false
    ...
    mounted() {
        /** 初始化拖拽上传 */
        const uploadEl = this.$refs.uploadInput as HTMLInputElement
    ...
    methods: {
        ...
        handleWrapEnter(e: KeyboardEvent) {
            if (e.target !== e.currentTarget) {
                return
            }
            if ((e.keyCode === 13 || e.keyCode === 32) && this.$refs.uploadInput) {
                const el_ = this.$refs.uploadInput as HTMLInputElement
                el_.value = ''
                el_.click()
            }
        },
        preHandleFile(file: File): FileObj {
            const fileObj = {
                name: file.name,
        ...
        uploadFile(fileObj: FileObj) {
            ...
            const uploadProgress = (e: ProgressEvent, val: string) => {
            ...
            const options = {
                ...
                onProgress: (event: ProgressEvent, val: string) => {
                    uploadProgress(event, val)
                },
                onSuccess: (reponse: XhrResponse, fileObj: FileObj) => {
                    this.handleSuccess(reponse, fileObj)
                },
                onError: (fileObj: FileObj, fileList: FileObj[], response: unknown) => {
                    this.$emit('on-error', fileObj, fileList, response)
                },
                onDone: (fileObj: FileObj) => {
        ...
        handleSuccess(response: XhrResponse, file: FileObj) {
        ...
        // 从文件列表中移除文件上传记录
        deleteFile(index: number, file: FileObj) {
            if (file.xhr) {
                file.xhr.abort()
            }
        ...
        // 快捷删除已上传的文件
        removeFile(index: number, file: FileObj) {
            this.$emit('on-remove', { index, file })
        },
        retry(index: number, file: FileObj) {
